...
useCSV(path, key_row, grade_row, name_row, usecodeid, reachedp_row, maxp_row) {
  ...
  for (final row in csv_parsed) {
    ...
    if (reachedp_row == 0 || maxp_row == 0) {
      if (usecodeid) {
        grades.add(
            "$name,$key,$grade,${csvReturnEncrypted(grade, key, true)}, $codeIDis");
      } else {
        grades
      ...
      if (usecodeid) {
        grades.add(
            "$name,$key,$grade,$reached,$max,${csvReturnEncryptedPoints(grade, reached, max, key, true)}, $codeIDis");
      } else {
        grades.add(
...
csvFormatPoints(encrypted, publicKey, reachedp, maxp, usecodeid) {
  ...
  if (!usecodeid) {
    ...
    encData =
        "r:${gradecryptPub(publicKey)}:${gradecrypt(encrypted)}:${gradecrypt(reachedp)}:${gradecrypt(maxp)}:${codeIDis}";
  }
...
csvFormat(encrypted, publicKey, usecodeid) {
  ...
  if (usecodeid) {
    encData =
        "i:${gradecryptPub(publicKey)}:${gradecrypt(encrypted)}:${codeIDis}";
  } else {
